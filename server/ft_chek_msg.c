...
static void	ft_leave_chanel(t_cli *cli, int nbr, bool flag)
{
	...
	if (flag)
	{
		if (ft_strcmp(cli[nbr].r_buff + ft_strlen(FT_LEAVE) + 1, cli[nbr].channel))
		{
...
void		ft_personal_message(t_cli *cli, int nbr)
{
	...
	while (++count < MAX_CLI)
	{
		...
		{
			msg = ft_msg(cli[nbr].name, cli[nbr].r_buff + i + ft_strlen(cli[count].name) + 1);
...
static void	ft_who(t_cli *cli, int nb)
{
	...
	while (++i < MAX_CLI)
	{
		if (cli[i].active == IS_CLI && !ft_strcmp(cli[nb].channel, cli[i].channel))
		{
